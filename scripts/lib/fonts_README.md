...
| **`calsans-var-flex`** | `CalSansFlex` | **Cal Sans Flex** — the morphing, cutting-ege build for the adventurous types. Along the `GEOM` axis compatible letterforms *blend* between forms instead of swapping, and `avar2` lengthens tall ascenders to small optical sizes. Great for movement in web emphasis like roll-over or animation. (`YTAS` is hidden as it follows `opsz` automatically.) |
